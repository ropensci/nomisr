...
`nomisr` is for accessing [Nomis](https://www.nomisweb.co.uk/) data with R. Nomis contains UK labour market statistics, data on benefits, demographic data and census data, and is maintained on behalf of the Office for National Statistics by the University of Durham.

`nomisr` returns data in [tibble](https://cran.r-project.org/package=tibble) format. Most of the data available through `nomisr` is based around statistical geographies, with some exceptions.
