...
#' the parameters. The wildcard character * can be added to the beginning 
#' and/or end of each search query.
#'
...
nomis_search <- function(name = NULL, description = NULL,
  ...
  if (length(description) > 0 ) {
    description_query <-     paste0("&search=description-", 
  ...
  if (length(keywords) > 0 ) {
    keywords_query <-     paste0("&search=keywords-", 
  ...
  if (length(content_type) > 0 ) {
    content_type_query <-     paste0("&search=contenttype-", 
  ...
  if (length(units) > 0 ) {
    units_query <-     paste0("&search=units-",
  ...
  query <- paste0("def.sdmx.json?", name_query, description_query, 
